...
Приложение НЕ обращается к этому сервису напрямую. Оно шлёт ~5-секундный клип
(`multipart/form-data`, поле `file`) на **Node-бэкенд** `POST /cry/analyze` с
токеном сессии; тот проксирует запрос сюда (`CRY_API_URL` → `POST
/api/v1/predict-cry`) и возвращает JSON. Так у приложения одна аутентифицированная
точка входа, а этот сервис остаётся внутренним (наружу выставлять не нужно).

Прямой вызов `POST /api/v1/predict-cry` тоже работает — удобно для отладки
(`curl`) и если понадобится обращаться к сервису без бэкенда.
