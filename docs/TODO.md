...
| 9.13 | Frames 15a–15e are **specified but not built**. The screen exists (`cry_insight_screen.dart`); the gaps are: no «Стоп» during recording (the button is nulled at `:181`, so a recording of her own child cannot be aborted), no countdown, no mic-denied action, no bottom action bar, no split between "the service is down" and "the audio was unreadable", and 15a is a flat nine-row sheet with no counts. See `docs/CLAUDE-app-design.md` § «Что эти пять кадров означают построчно» for the decided order, states and copy. | Open. The 502/503 wording is the urgent half: with no `model.pkl` every recording today ends in «попробуйте ещё раз в тишине». |
