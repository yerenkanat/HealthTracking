...
| 10.5 | **`verify_datemath` is red and left red** — 3 hits, the load-bearing one `home_shell.dart:821`, `picked.add(Duration(days: 280))`: 280 × 24 h of *elapsed* time, not 280 calendar days, on the value every pregnancy week, antenatal window and labour date is anchored to. Kazakhstan has had no DST since 2005 so it does not bite in this market — the guard exists so nobody has to know that. | Open. |
| 10.6 | **`verify_destructive` is red on a false positive**, `womens_health_screen.dart:993` — the undo handler of the "baby was born" transition, which belongs in the tool's `_exempt` list. The finding is the red guard: while it is red, a genuinely unconfirmed delete added tomorrow is indistinguishable from this. | Open. |
...
| 11.11 | `GET /account/entitlements` has no caller — the only orphan among 213 registered routes. The app derives entitlement from `/course/lessons`. Live and authenticated. | Open. |
