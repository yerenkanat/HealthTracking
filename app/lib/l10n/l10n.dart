...
const Map<String, Map<AppLocale, String>> _catalog = {
  ...
  'legal_priv_collect_b': {
    ...
    AppLocale.kk: 'Сіз туралы: атыңыз, телефон нөміріңіз, тіліңіз, уақыт белдеуіңіз, қаласаңыз — туған күніңіз, қалаңыз, үй мекенжайыңыз және дәрігеріңіздің телефоны. Мекенжай бір ғана нәрсеге керек: жедел жәрдемді шақыратын жер болуы үшін.\n\nЖүктілік пен денсаулық туралы: болжамды босану күні, цикл ұзақтығы, күн күнделігі (көңіл-күй, симптомдар, бөліністер, еркін жазба), қимылдар, толғақ, салмақ, дәрілер және оларды қабылдау, дәрігер қабылдаулары — атауы мен жазбасымен, түнгі ұйқы, босанғаннан кейінгі депрессия тестінің қорытынды баллы.\n\nБілезіктен: дене қызуы, тамыр соғуы, қанықтық, қан қысымы, қант, қадам, калория, қашықтық, күйзеліс, тыныс алу, заряд және тағылып тұрғанының белгісі — сондай-ақ қолданба осы көрсеткіштер бойынша есептеген алаңдаушылық дәрежесі.\n\nБала туралы: аты, жынысы, туған күні, күні көрсетілген бойы мен салмағы, егулер туралы белгілер, тамақтандыру, ұйқы және жаялық, сондай-ақ төтенше жағдайға арналған карта: қан тобы, аллергиялар, созылмалы аурулар, дәрілер, дәрігердің және сенімді ересек адамның аты мен телефоны.\n\nҚауіпсіздік туралы: картада өзіңіз белгілеген аймақтар — яғни үйіңіз бен мектептің координаттары — және аймақ атауы мен уақыты көрсетілген «аймақтан шықты», «келді», «SOS басылды», «брелоктың заряды отырды» оқиғалары.\n\nҚызметтік: жалғанған құрылғының идентификаторы мен микробағдарламасы, кіру жазбалары, телефон нөмірімен кіру әрекеттері.\n\nТапсырыстар: аты, телефоны, қаласы, жеткізу мекенжайы және түсініктеме. Банк картасының деректері бізде еш жерде жоқ — біз оларды алмаймыз да, сақтамаймыз да.',
    AppLocale.en: 'About you: name, phone number, language, time zone and, if you choose, date of birth, city, home address and your doctor’s phone. The address serves one purpose: somewhere to send an ambulance.\n\nPregnancy and health: due date, cycle length, the day diary (mood, symptoms, flow, free-text note), fetal movements, contractions, weight, medications and doses taken, appointments with their title and note, nightly sleep, and the resulting postnatal depression score.\n\nFrom the band: temperature, heart rate, oxygen saturation, blood pressure, blood sugar, steps, calories, distance, stress, breathing rate, battery and whether it is being worn — plus the severity the app computed from those readings.\n\nAbout the child: name, sex, date of birth, height and weight by date, vaccination ticks, feeds, sleep and nappies, and the emergency card: blood type, allergies, chronic conditions, medications, and the name and phone of the doctor and of a trusted adult.\n\nSafety: the zones you marked on the map — that is, the coordinates of your home and school — and the events “left the zone”, “arrived”, “pressed SOS”, “tag battery low”, with the zone name and time.\n\nTechnical: the identifier and firmware of a paired device, sign-in records, and sign-in attempts with the phone number.\n\nOrders: name, phone, city, delivery address and comment. We hold no bank card data anywhere — we neither receive nor store it.',
  },
  'legal_priv_storage_h': {AppLocale.ru: 'Где эти данные лежат', AppLocale.kk: 'Бұл деректер қайда жатыр', AppLocale.en: 'Where the data is held'},
  'legal_priv_storage_b': {
    AppLocale.ru: 'Раньше здесь было написано, что по умолчанию данные остаются на телефоне. Это перестало быть правдой, и мы это исправляем, а не переписываем помягче.\n\nКак есть на самом деле: пока вы вошли в аккаунт, приложение отправляет на наш сервер профиль, детей, приёмы, лекарства, вес, дневник цикла, сон, баллы теста, шевеления, схватки, рост и вес ребёнка, прививки, дневник новорождённого, экстренную карточку ребёнка, результаты определения плача, оповещения безопасности и все показатели с браслета. На сервере лежит полная копия, и именно она возвращается вам на новом телефоне.\n\nЧто сегодня НЕ уходит с телефона: подробный маршрут ребёнка. Точки перемещения считаются и рисуются на вашем устройстве. Сами события «вышел из зоны» и «нажал SOS» на сервер уходят — маршрут между ними нет. Если это изменится, мы изменим и этот абзац до того, как включим отправку, а не после.',
    AppLocale.kk: 'Бұрын мұнда деректер әдепкі бойынша телефонда қалады деп жазылған еді. Ол шындық болудан қалды, сондықтан біз оны жұмсартып қайта жазбай, түзетіп отырмыз.\n\nШындығында былай: аккаунтқа кіріп тұрғаныңызда қолданба біздің серверге профиліңізді, балаларыңызды, дәрігер қабылдауларын, дәрілерді, салмақты, цикл күнделігін, ұйқыны, тест баллдарын, қимылдарды, толғақты, баланың бойы мен салмағын, егулерді, нәресте күнделігін, баланың төтенше картасын, жылау себебін анықтау нәтижелерін, қауіпсіздік хабарламаларын және білезіктің барлық көрсеткішін жібереді. Серверде толық көшірме жатады, жаңа телефонда сізге қайтып келетіні — дәл сол көшірме.\n\nБүгінде телефоннан ШЫҚПАЙТЫНЫ: баланың толық жүрген жолы. Орын нүктелері сіздің құрылғыңызда есептеледі және сонда сызылады. «Аймақтан шықты» мен «SOS басылды» оқиғаларының өзі серверге кетеді — ал олардың арасындағы жол кетпейді. Бұл өзгерсе, біз жіберуді қосқанға дейін осы азатжолды да өзгертеміз, кейін емес.',
    AppLocale.en: 'This section used to say that by default your data stays on your phone. That stopped being true, and we are correcting it rather than softening it.\n\nWhat actually happens: while you are signed in, the app sends to our server your profile, children, appointments, medications, weight, cycle diary, sleep, screening scores, fetal movements, contractions, the child’s height and weight, vaccination ticks, the newborn diary, the child’s emergency card, cry-analysis results, safety alerts and every reading from the band. The server holds a full copy, and that copy is what comes back to you on a new phone.\n\nWhat does NOT leave the phone today: the child’s detailed route. Position points are computed and drawn on your device. The events themselves — “left the zone”, “pressed SOS” — do go to the server; the trail between them does not. If that changes, we will change this paragraph before we switch the sending on, not after.',
  },
  'legal_priv_medical_h': {AppLocale.ru: 'Это не медицинское изделие', AppLocale.kk: 'Бұл — медициналық бұйым емес', AppLocale.en: 'This is not a medical device'},
  'legal_priv_medical_b': {
    AppLocale.ru: 'Ana-Bala не ставит диагнозов и не заменяет врача. Браслет — бытовое устройство, а не сертифицированный прибор: его давление, сатурация и температура являются оценкой, а не измерением, и на них нельзя опираться при решении о лечении.\n\nПодсказки приложения основаны на общих рекомендациях и на ваших записях. Они не учитывают вашу историю болезни целиком.\n\nПри любом тревожном признаке звоните 103.',
    AppLocale.kk: 'Ana-Bala диагноз қоймайды және дәрігердің орнын баспайды. Білезік — тұрмыстық құрылғы, сертификатталған аспап емес: оның қан қысымы, қанықтық және дене қызуы — болжам, өлшем емес, сондықтан емдеу туралы шешімде оларға сүйенуге болмайды.\n\nҚолданбаның кеңестері жалпы ұсынымдарға және сіздің жазбаларыңызға негізделген. Олар ауру тарихыңызды толық ескермейді.\n\nКез келген қауіпті белгіде 103-ке қоңырау шалыңыз.',
    AppLocale.en: 'Ana-Bala does not diagnose and does not replace a doctor. The band is a consumer device, not a certified instrument: its blood pressure, oxygen saturation and temperature are estimates rather than measurements, and no treatment decision should rest on them.\n\nThe app’s prompts are based on general guidance and on your own entries. They do not take your full medical history into account.\n\nAt any worrying sign, call 103.',
  },
  ...
  'legal_terms_medical_h': {
    ...
    AppLocale.kk: 'Қолданба дәрігердің орнын баспайды',
    AppLocale.en: 'The app does not replace a doctor',
  },
  'legal_terms_medical_b': {
    AppLocale.ru: 'Это главный пункт этих условий, и он важнее остальных.\n\nAna-Bala не является медицинским изделием, не ставит диагнозов и не назначает лечения. Браслет и брелок — бытовая электроника, а не сертифицированные приборы. Давление, сатурация, температура и сахар, которые они показывают, — оценка по датчику, а не измерение; они могут расходиться с показаниями поверенного прибора и с истиной.\n\nНикакое решение о приёме, отмене или изменении лекарства не должно приниматься по показаниям приложения. Такие решения принимает врач.\n\nОтсутствие тревожного сообщения не означает, что всё в порядке. Приложение видит только то, что вы в него внесли и что успел передать браслет.',
    AppLocale.kk: 'Бұл — осы шарттардың басты тармағы, ол қалғандарының бәрінен маңызды.\n\nAna-Bala медициналық бұйым болып табылмайды, диагноз қоймайды және ем тағайындамайды. Білезік пен брелок — тұрмыстық электроника, сертификатталған аспаптар емес. Олар көрсететін қан қысымы, қанықтық, дене қызуы және қант — датчиктің болжамы, өлшем емес; олар тексеруден өткен аспаптың көрсеткішінен де, шындықтан да алшақ болуы мүмкін.\n\nДәріні қабылдау, тоқтату немесе өзгерту туралы ешбір шешім қолданбаның көрсеткіштері бойынша қабылданбауға тиіс. Мұндай шешімдерді дәрігер қабылдайды.\n\nЕскерту хабарламасының болмауы бәрі жақсы дегенді білдірмейді. Қолданба тек өзіңіз енгізгенді және білезік жіберіп үлгергенді ғана көреді.',
    AppLocale.en: 'This is the principal clause of these terms and it matters more than the others.\n\nAna-Bala is not a medical device, does not diagnose and does not prescribe. The band and the tag are consumer electronics, not certified instruments. The blood pressure, oxygen saturation, temperature and blood sugar they display are sensor estimates rather than measurements; they may differ from a calibrated instrument and from the truth.\n\nNo decision to take, stop or change a medication should be made on what the app shows. A doctor makes those decisions.\n\nThe absence of an alarming message does not mean all is well. The app sees only what you have entered into it and what the band has managed to send.',
  },
  'legal_terms_emergency_h': {
    AppLocale.ru: 'Экстренные функции — не служба спасения',
    AppLocale.kk: 'Төтенше функциялар — құтқару қызметі емес',
    AppLocale.en: 'Emergency features are not an emergency service',
  },
  'legal_terms_emergency_b': {
    AppLocale.ru: 'Кнопка SOS, оповещения о выходе из зоны и предупреждения о показателях — вспомогательные функции. Это не вызов скорой и не охранная служба, и никто не дежурит у экрана круглосуточно, чтобы прийти на помощь по сигналу из приложения.\n\nВ опасной ситуации звоните 103 (скорая) или 102 (полиция).\n\nОповещение может не дойти или прийти с опозданием: у телефона может сесть батарея, пропасть интернет, у брелока — разрядиться аккумулятор, а система может ограничить работу приложения в фоне. Строить безопасность ребёнка на одном лишь приложении нельзя.',
    AppLocale.kk: 'SOS түймесі, аймақтан шығу туралы хабарламалар және көрсеткіштер жөніндегі ескертулер — көмекші функциялар. Бұл — жедел жәрдем шақыруы да, күзет қызметі де емес; қолданбадан келген сигнал бойынша көмекке жету үшін экран алдында тәулік бойы кезекші отырған ешкім жоқ.\n\nҚауіпті жағдайда 103-ке (жедел жәрдем) немесе 102-ге (полиция) қоңырау шалыңыз.\n\nХабарлама жетпей қалуы немесе кешігіп келуі мүмкін: телефонның батареясы отыруы, интернет үзілуі, брелоктың аккумуляторы таусылуы мүмкін, ал жүйе қолданбаның фондағы жұмысын шектеуі мүмкін. Баланың қауіпсіздігін тек қолданбаға ғана сүйеп қоюға болмайды.',
    AppLocale.en: 'The SOS button, zone alerts and readings warnings are supporting features. They are not an ambulance call and not a security service, and nobody is on duty at a screen around the clock to respond to a signal from the app.\n\nIn a dangerous situation call 103 (ambulance) or 102 (police).\n\nAn alert may not arrive, or may arrive late: a phone’s battery can die, the internet can drop, a tag can run flat, and the operating system can restrict the app in the background. A child’s safety must not rest on the app alone.',
  },
  'legal_terms_responsib_h': {AppLocale.ru: 'Ваша ответственность', AppLocale.kk: 'Сіздің жауапкершілігіңіз', AppLocale.en: 'Your responsibility'},
  'legal_terms_responsib_b': {
    AppLocale.ru: 'Вы отвечаете за достоверность внесённых данных, за сохранность доступа к своему номеру телефона и за то, кому вы открыли доступ к данным ребёнка.\n\nВы отвечаете и за решения, которые принимаете, увидев что-либо в приложении. Приложение показывает и подсказывает — решает человек, и при сомнении это должен быть врач.',
    AppLocale.kk: 'Енгізген деректеріңіздің дұрыстығы үшін, өз телефон нөміріңізге қолжетімділікті сақтағаныңыз үшін және бала деректеріне кімге рұқсат бергеніңіз үшін сіз жауап бересіз.\n\nҚолданбадан бірдеңе көріп қабылдаған шешімдеріңіз үшін де сіз жауаптысыз. Қолданба көрсетеді және кеңес береді — шешімді адам қабылдайды, ал күмән болса, ол адам дәрігер болуға тиіс.',
    AppLocale.en: 'You are responsible for the accuracy of what you enter, for keeping control of your phone number, and for whom you have given access to your child’s data.\n\nYou are also responsible for the decisions you make on seeing something in the app. The app shows and suggests — a person decides, and where there is doubt that person should be a doctor.',
  },
  'legal_terms_warranty_h': {AppLocale.ru: 'Пределы нашей ответственности', AppLocale.kk: 'Біздің жауапкершілігіміздің шегі', AppLocale.en: 'Limits of our liability'},
  'legal_terms_warranty_b': {
    AppLocale.ru: 'Сервис предоставляется как есть. Мы не гарантируем, что оценки браслета совпадут с показаниями медицинского прибора, что оповещение придёт вовремя и что подсказка окажется применима именно к вам.\n\nМы не отвечаем за вред, наступивший оттого, что решение о здоровье было принято по данным приложения вместо обращения к врачу, и за последствия неполученного или запоздавшего оповещения.\n\nЭти ограничения действуют в пределах, допускаемых законодательством Республики Казахстан. Ответственность за вред жизни и здоровью, причинённый по нашей вине, ими не исключается.',
    AppLocale.kk: 'Сервис қалай бар, солай ұсынылады. Білезіктің болжамдары медициналық аспаптың көрсеткішімен сәйкес келетініне, хабарламаның уақытында жететініне және кеңестің дәл сізге келетініне кепілдік бермейміз.\n\nДәрігерге жүгінудің орнына денсаулық туралы шешім қолданба деректері бойынша қабылданғандықтан келген зиян үшін де, жетпей қалған немесе кешіккен хабарламаның салдары үшін де жауап бермейміз.\n\nБұл шектеулер Қазақстан Республикасының заңнамасы жол берген шекте қолданылады. Біздің кінәмізден өмір мен денсаулыққа келтірілген зиян үшін жауапкершілік олармен жойылмайды.',
    AppLocale.en: 'The service is provided as is. We do not guarantee that the band’s estimates will match a medical instrument, that an alert will arrive in time, or that a piece of guidance will apply to you specifically.\n\nWe are not liable for harm arising because a health decision was made on the app’s data instead of consulting a doctor, nor for the consequences of an alert that did not arrive or arrived late.\n\nThese limits apply to the extent permitted by the legislation of the Republic of Kazakhstan. They do not exclude liability for harm to life and health caused through our fault.',
  },
  'legal_terms_law_h': {AppLocale.ru: 'Применимое право', AppLocale.kk: 'Қолданылатын құқық', AppLocale.en: 'Governing law'},
  'legal_terms_law_b': {
    AppLocale.ru: 'К этим условиям применяется право Республики Казахстан.\n\nСпоры мы сначала стараемся решить перепиской. Если не выходит — они рассматриваются судами Республики Казахстан по правилам подсудности. Ваши права потребителя этим не ограничиваются.',
    AppLocale.kk: 'Осы шарттарға Қазақстан Республикасының құқығы қолданылады.\n\nДауларды алдымен хат алмасу арқылы шешуге тырысамыз. Шешілмесе — олар соттылық ережелері бойынша Қазақстан Республикасының соттарында қаралады. Тұтынушы ретіндегі құқықтарыңыз бұнымен шектелмейді.',
    AppLocale.en: 'These terms are governed by the law of the Republic of Kazakhstan.\n\nWe first try to resolve disputes by correspondence. Failing that, they are heard by the courts of the Republic of Kazakhstan under the applicable rules of jurisdiction. Your consumer rights are not restricted by this.',
  },
  ...
  'legal_priv_short_b': {
    AppLocale.ru: '• Пока вы вошли в аккаунт, копия ваших записей хранится на нашем сервере, а не только на телефоне.\n• Мы не продаём данные и не показываем рекламу. В приложении нет ни системы аналитики, ни сборщика сбоев — никакой сторонний счётчик за вами не следит.\n• Запись плача уходит на анализ и нигде не сохраняется. Остаётся только слово-причина и уверенность в процентах.\n• Ответы теста на послеродовую депрессию не сохраняются никогда. Сохраняется только итоговый балл.\n• Рассылки мы никогда не строим по вашим показателям, дневнику цикла или баллу теста.\n• Аккаунт можно удалить из приложения, и он удаляется на сервере.',
    AppLocale.kk: '• Аккаунтқа кіріп тұрғаныңызда жазбаларыңыздың көшірмесі тек телефонда емес, біздің серверде де сақталады.\n• Деректерді сатпаймыз және жарнама көрсетпейміз. Қолданбада аналитика жүйесі де, ақау жинағыш та жоқ — сізді бақылап отырған бөгде санауыш жоқ.\n• Жылау жазбасы талдауға жіберіледі және еш жерде сақталмайды. Тек себеп-сөз бен пайызбен берілген сенімділік қалады.\n• Босанғаннан кейінгі депрессия тестінің жауаптары ешқашан сақталмайды. Тек қорытынды балл сақталады.\n• Таратылымдарды ешқашан көрсеткіштеріңіз, цикл күнделігіңіз немесе тест балыңыз бойынша құрмаймыз.\n• Аккаунтты қолданбадан жоюға болады, ол серверде де жойылады.',
    AppLocale.en: '• While you are signed in, a copy of your records is held on our server, not only on your phone.\n• We do not sell data and we show no advertising. The app contains no analytics system and no crash reporter — no third-party tracker follows you.\n• A cry recording is sent for analysis and is stored nowhere. Only a reason word and a confidence percentage remain.\n• Your answers to the postnatal depression questionnaire are never stored. Only the resulting score is.\n• We never build message audiences from your readings, your cycle diary or your screening score.\n• You can delete your account from the app, and it is deleted on the server.',
  },
  'legal_priv_child_h': {AppLocale.ru: 'Данные ребёнка', AppLocale.kk: 'Бала деректері', AppLocale.en: 'Your child’s data'},
  'legal_priv_child_b': {
    AppLocale.ru: 'Имя ребёнка, дата рождения, группа крови, аллергии и координаты его зон — самые чувствительные сведения во всём приложении, потому что вместе они позволяют узнать конкретного ребёнка и понять, где он бывает. Мы обращаемся с ними строже, чем с остальным.\n\nВы вносите их как родитель или законный представитель и отвечаете за то, что вправе это делать.\n\nДоступ к данным ребёнка можно дать второму взрослому — например, отцу или бабушке. Такой доступ распространяется только на ребёнка: местоположение, зоны, оповещения и устройство. Никакой уровень доступа не открывает вашу собственную медицинскую карту — этого нельзя выразить в системе даже при желании.\n\nПриглашение действует сутки, используется один раз, и одновременно их может быть не больше десяти. Выданный доступ можно отозвать в любой момент.',
    AppLocale.kk: 'Баланың аты, туған күні, қан тобы, аллергиялары және оның аймақтарының координаттары — бүкіл қолданбадағы ең нәзік мәліметтер, өйткені бірге алғанда олар нақты бір баланы тануға және оның қайда жүретінін білуге мүмкіндік береді. Біз оларға қалғанынан гөрі қаталырақ қараймыз.\n\nОларды ата-ана немесе заңды өкіл ретінде енгізесіз және бұған құқығыңыз бар екені үшін жауап бересіз.\n\nБала деректеріне екінші ересек адамға — мысалы, әкесіне немесе әжесіне — рұқсат беруге болады. Мұндай рұқсат тек балаға қатысты: орналасқан жері, аймақтар, хабарламалар және құрылғы. Ешбір рұқсат деңгейі сіздің өз медициналық картаңызды ашпайды — тіпті қалаған күннің өзінде де жүйе мұндай рұқсатты бере алмайды.\n\nШақыру бір тәулік бойы жарамды, бір рет қана қолданылады, әрі бір мезгілде олардың саны оннан аспайды. Берілген рұқсатты кез келген сәтте кері қайтаруға болады.',
    AppLocale.en: 'A child’s name, date of birth, blood type, allergies and the coordinates of their zones are the most sensitive information in the app, because together they identify a specific child and show where they go. We treat them more strictly than the rest.\n\nYou enter them as a parent or legal guardian and are responsible for being entitled to do so.\n\nYou can grant a second adult access to a child’s data — a father or a grandmother, say. That access covers the child only: location, zones, alerts and the device. No level of access opens your own medical record; the system cannot express such a grant even if someone wanted to.\n\nAn invitation is valid for 24 hours, can be used once, and no more than ten may be open at a time. Access you have granted can be revoked at any moment.',
  },
  'legal_priv_cry_h': {AppLocale.ru: 'Определение причины плача', AppLocale.kk: 'Жылау себебін анықтау', AppLocale.en: 'Cry analysis'},
  'legal_priv_cry_b': {
    AppLocale.ru: 'Здесь важна точность, поэтому опишем всю цепочку.\n\nКогда вы нажимаете запись, приложение записывает короткий фрагмент плача во временный файл на вашем телефоне и отправляет его на наш сервер. Сервер не сохраняет его, а передаёт распознающему сервису. Тот раскладывает звук в памяти, возвращает ответ и не пишет его на диск. Файл со звуком не сохраняется нигде, и в базе данных его нет — такой колонки просто не существует. Временный файл на телефоне удаляется сразу после отправки — и в том случае, если отправка не удалась или вы прервали запись.\n\nСохраняется только результат: время, слово-причина (голоден, устал, дискомфорт, боль), уверенность в процентах и, если вы отметите, была ли догадка верной.\n\nМы говорим это прямо, потому что соблазн написать «звук не покидает телефон» велик, а это была бы неправда: звук телефон покидает. Он не сохраняется — это разные вещи, и путать их нельзя. То, что запись не сохраняется, проверяется автоматическим тестом при каждой сборке.',
    AppLocale.kk: 'Мұнда дәлдік маңызды, сондықтан жылау себебін анықтаудың бүкіл тізбегін сипаттап береміз.\n\nЖазуды бастаған кезде қолданба телефоныңыздағы уақытша файлға жылаудың қысқа үзіндісін жазып алады да, оны біздің серверге жібереді. Сервер оны сақтамайды, тануыш сервиске береді. Ол дыбысты жадыда талдап, жауап қайтарады және оны дискіге жазбайды. Дыбыс файлы еш жерде сақталмайды, дерекқорда да жоқ — ондай баған мүлде жоқ. Телефондағы уақытша файл жіберілген бойда жойылады — жіберу сәтсіз аяқталған немесе жазуды өзіңіз тоқтатқан жағдайда да.\n\nТек нәтиже сақталады: уақыты, себеп-сөз (аштық, шаршау, ыңғайсыздық, ауырсыну), пайызбен берілген сенімділік және, өзіңіз белгілесеңіз, болжамның дұрыс болған-болмағаны.\n\nМұны ашық айтамыз, өйткені «дыбыс телефоннан шықпайды» деп жазу азғырығы күшті, ал бұл — жалған болар еді: дыбыс телефоннан шығады. Ол сақталмайды — бұл екеуі екі бөлек нәрсе, оларды шатастыруға болмайды. Жазбаның сақталмайтынын әрбір құрастыру кезінде автоматты тест тексереді.',
    AppLocale.en: 'Precision matters here, so we describe the whole chain.\n\nWhen you press record, the app records a short fragment of your baby’s cry to a temporary file on your phone and sends it to our server. The server does not save it; it passes it to the recognition service. That service decomposes the sound in memory, returns an answer, and writes nothing to disk. The audio file is stored nowhere, and it is not in the database — no such column exists. The temporary file on your phone is deleted as soon as the clip has been sent — and also if the sending failed, or if you abandoned the recording partway.\n\nOnly the result is stored: the time, a reason word (hungry, tired, discomfort, pain), a confidence percentage and, if you tell us, whether the guess was right.\n\nWe say this plainly because the temptation to write “the sound never leaves your phone” is strong, and it would be untrue: the sound does leave your phone. It is not stored — those are different things and must not be conflated. That the recording is not stored is checked by an automated test on every build.',
  },
  'legal_priv_ai_h': {AppLocale.ru: 'Помощник и снимки приборов', AppLocale.kk: 'Көмекші және аспаптардың суреттері', AppLocale.en: 'The assistant and photos of devices'},
  'legal_priv_ai_b': {
    AppLocale.ru: 'В приложении есть помощник, которому можно написать вопрос, и возможность сфотографировать тонометр, глюкометр, бланк анализа, коробку с лекарством или талон к врачу, чтобы не набирать цифры руками.\n\nТекст вашего вопроса и такие фотографии передаются компании Anthropic (США), чей сервис их разбирает. Показатели с браслета в переписку с помощником не отправляются — степень тревожности считается у нас.\n\nПожалуйста, помните: на бланке анализа обычно напечатаны ваши фамилия и имя. Фотографируя его, вы отправляете и их.',
    AppLocale.kk: 'Қолданбада сұрақ жазуға болатын көмекші бар, сондай-ақ сандарды қолмен термеу үшін тонометрді, глюкометрді, талдау бланкісін, дәрі қорабын немесе дәрігерге жазылу талонын суретке түсіру мүмкіндігі бар.\n\nСұрағыңыздың мәтіні мен осындай суреттер Anthropic компаниясына (АҚШ) беріледі, оның сервисі оларды талдайды. Білезіктің көрсеткіштері көмекшімен жазысуға жіберілмейді — алаңдаушылық дәрежесі бізде есептеледі.\n\nЕсіңізде болсын: талдау бланкісінде әдетте тегіңіз бен атыңыз басылып тұрады. Оны суретке түсіргенде, оларды да жібересіз.',
    AppLocale.en: 'The app has an assistant you can write a question to, and the ability to photograph a blood-pressure monitor, a glucometer, a lab form, a medicine box or an appointment slip so you need not type the numbers.\n\nThe text of your question and such photographs are transferred to Anthropic (USA), whose service analyses them. Band readings are not sent to the assistant — severity is computed on our side.\n\nPlease remember that a lab form usually has your surname and first name printed on it. Photographing it sends those too.',
  },
  'legal_priv_share_h': {AppLocale.ru: 'Кому мы передаём данные', AppLocale.kk: 'Деректерді кімге береміз', AppLocale.en: 'Who we share data with'},
  'legal_priv_share_b': {
    AppLocale.ru: 'Мы не продаём персональные данные и не передаём их для чужой рекламы. Список тех, кто их всё же получает, короткий и полный:\n\n• Anthropic (США) — текст вопросов помощнику и фотографии приборов и бланков, как описано выше.\n• Telegram — если вы оставили заявку на сайте, ваше имя, телефон, выбранный набор и язык приходят нам уведомлением в Telegram.\n• Google — карты в приложении и видео курса, встроенные с YouTube. Google при этом видит ваш IP-адрес и то, какой участок карты вы смотрите, то есть примерно ваш район.\n• WhatsApp (Meta) — если вы пишете нам в WhatsApp. В заготовленное сообщение поддержки подставляются версия приложения, ваш номер, идентификатор устройства и тип связи; ничего медицинского туда не подставляется.\n• Хостинг — сервер, на котором работают сервис и база данных.\n\nЧего в этом списке нет и почему: у нас нет системы аналитики, нет сборщика сбоев, не подключена рассылка SMS и не подключены push-уведомления Google — токены уведомлений не собираются вовсе. Оплата картой через сервис не проходит: ссылка на Kaspi — это просто ссылка, и никаких данных мы туда не передаём. Часы и брелок работают только по Bluetooth, у их производителя нет облака, в которое что-то уходило бы.',
    AppLocale.kk: 'Дербес деректерді сатпаймыз және бөгде жарнама үшін бермейміз. Оларды бәрібір алатындардың тізімі қысқа әрі толық:\n\n• Anthropic (АҚШ) — жоғарыда сипатталғандай, көмекшіге қойылған сұрақтардың мәтіні мен аспаптар мен бланкілердің суреттері.\n• Telegram — сайтта өтінім қалдырсаңыз, атыңыз, телефоныңыз, таңдаған жинағыңыз және тіліңіз бізге Telegram хабарламасымен келеді.\n• Google — қолданбадағы карталар және YouTube-тан кірістірілген курс бейнелері. Бұл ретте Google сіздің IP-мекенжайыңызды және картаның қай бөлігін қарап отырғаныңызды, яғни шамамен қай ауданда екеніңізді көреді.\n• WhatsApp (Meta) — бізге WhatsApp арқылы жазсаңыз. Қолдауға арналған дайын хабарламаға қолданба нұсқасы, нөміріңіз, құрылғы идентификаторы және байланыс түрі қойылады; оған медициналық ештеңе қойылмайды.\n• Хостинг — сервис пен дерекқор жұмыс істейтін сервер.\n\nБұл тізімде не жоқ және неге: бізде аналитика жүйесі жоқ, ақау жинағыш жоқ, SMS таратылымы қосылмаған және Google push-хабарламалары қосылмаған — хабарлама токендері мүлде жиналмайды. Картамен төлем сервис арқылы өтпейді: Kaspi сілтемесі — жай ғана сілтеме, оған ешқандай дерек бермейміз. Сағат пен брелок тек Bluetooth арқылы жұмыс істейді, олардың өндірушісінде бірдеңе кететін бұлт жоқ.',
    AppLocale.en: 'We do not sell personal data and do not pass it on for anyone else’s advertising. The list of those who do receive it is short and complete:\n\n• Anthropic (USA) — the text of assistant questions and photographs of devices and forms, as described above.\n• Telegram — if you leave a request on the website, your name, phone, chosen package and language reach us as a Telegram notification.\n• Google — maps in the app and course videos embedded from YouTube. Google therefore sees your IP address and which part of the map you are viewing, i.e. roughly your neighbourhood.\n• WhatsApp (Meta) — if you write to us on WhatsApp. The prepared support message includes the app version, your number, a device identifier and your connection type; nothing medical is included.\n• Hosting — the server on which the service and its database run.\n\nWhat is not on this list, and why: we have no analytics system, no crash reporter, no SMS gateway connected and no Google push notifications connected — notification tokens are not collected at all. No card payment passes through the service: the Kaspi link is simply a link, and we send no data to it. The watch and the tag work over Bluetooth only; their manufacturer has no cloud for anything to go to.',
  },
  'legal_priv_transfer_h': {AppLocale.ru: 'Передача за пределы Казахстана', AppLocale.kk: 'Қазақстаннан тыс жерге беру', AppLocale.en: 'Transfer outside Kazakhstan'},
  'legal_priv_transfer_b': {
    AppLocale.ru: 'Три вещи из перечисленных выше пересекают границу: вопросы помощнику и фотографии — в США, заявки с сайта — в Telegram, обращения к картам и видео — к Google.\n\nСтрана размещения основного сервера указывается здесь после юридической проверки. Мы не пишем её наугад: закон предъявляет отдельные требования к передаче данных граждан Казахстана за рубеж, и утверждение, которое мы не можем подтвердить, хуже, чем пустое место.',
    AppLocale.kk: 'Жоғарыда аталғандардың үшеуі шекарадан өтеді: көмекшіге қойылған сұрақтар мен суреттер — АҚШ-қа, сайттан келген өтінімдер — Telegram-ға, карталар мен бейнелерге жасалған сұраныстар — Google-ға.\n\nНегізгі сервер орналасқан ел мұнда заңгерлік сараптамадан кейін көрсетіледі. Оны ойдан жазбаймыз: заң Қазақстан азаматтарының деректерін шетелге беруге бөлек талаптар қояды, ал растай алмайтын тұжырым бос орыннан да жаман.',
  ...
  'legal_priv_rights_b': {
    AppLocale.ru: 'Посмотреть и исправить. Всё, что мы о вас храним, вы видите в приложении и можете изменить там же.\n\nЗабрать копию. В настройках есть выгрузка: приложение соберёт ваши записи в один файл и предложит сохранить или переслать его. Файл содержит ваши данные и не содержит ключа от аккаунта — переслать его безопасно.\n\nУдалить аккаунт. Кнопка в настройках удаляет ваши записи и на телефоне, и на сервере. Если сервер в этот момент недоступен, приложение скажет об этом прямо, а не сделает вид, что всё прошло. Что переживает удаление — написано выше.\n\nОтозвать согласие. Согласие на обработку можно отозвать, удалив аккаунт. Отдельные вещи выключаются по отдельности: доступ к микрофону, к геолокации, уведомления и доступ, выданный второму взрослому.\n\nПожаловаться. Если вы считаете, что мы обошлись с вашими данными неправильно, напишите нам, а если ответ вас не устроит — в уполномоченный орган по защите персональных данных Республики Казахстан.',
    AppLocale.kk: 'Көру және түзету. Сіз туралы сақтап отырғанымыздың бәрін қолданбадан көресіз және сол жерде өзгерте аласыз.\n\nКөшірмесін алу. Параметрлерде шығару бар: қолданба жазбаларыңызды бір файлға жинап, оны сақтауды немесе жіберуді ұсынады. Файлда деректеріңіз бар, аккаунтыңыздың кілті жоқ — оны жіберу қауіпсіз.\n\nАккаунтты жою. Параметрлердегі түйме жазбаларыңызды телефоннан да, серверден де жояды. Сол сәтте сервер қолжетімсіз болса, қолданба бәрі сәтті өтті деген кейіп танытпай, мұны тура айтады. Жойылудан не аман қалатыны жоғарыда жазылған.\n\nКелісімді кері қайтару. Өңдеуге берілген келісімді аккаунтты жою арқылы кері қайтаруға болады. Жекелеген нәрселер бөлек-бөлек өшіріледі: микрофонға және геолокацияға қолжетімділік, хабарламалар және екінші ересек адамға берілген рұқсат.\n\nШағымдану. Деректеріңізбен дұрыс жұмыс істемедік деп есептесеңіз, бізге жазыңыз, ал жауабымыз көңіліңізден шықпаса — Қазақстан Республикасының дербес деректерді қорғау жөніндегі уәкілетті органына жүгініңіз.',
    AppLocale.en: 'See and correct. Everything we hold about you is visible in the app and can be changed there.\n\nTake a copy. Settings offers an export: the app gathers your records into a single file and offers to save or forward it. The file contains your data and does not contain a key to your account — it is safe to forward.\n\nDelete your account. A button in Settings deletes your records both on the phone and on the server. If the server is unreachable at that moment, the app says so plainly instead of pretending it went through. What survives deletion is written above.\n\nWithdraw consent. Consent to processing can be withdrawn by deleting your account. Individual things can be switched off individually: microphone access, location access, notifications, and access granted to a second adult.\n\nComplain. If you believe we have handled your data wrongly, write to us — and if our answer does not satisfy you, to the authorised personal-data protection body of the Republic of Kazakhstan.',
  },
  'legal_priv_staff_h': {AppLocale.ru: 'Кто у нас видит эти данные', AppLocale.kk: 'Бұл деректерді бізде кім көреді', AppLocale.en: 'Who inside our company sees this'},
  'legal_priv_staff_b': {
    AppLocale.ru: 'Сотрудник видит только то, что нужно для его работы, и права выданы по задаче, а не по должности.\n\nПродавец и склад не видят ни одной записи о здоровье. Бухгалтерия видит деньги и не видит людей. Врач видит здоровье и не видит денег.\n\nЧтобы открыть карточку конкретной женщины — её сон, дневник цикла, лекарства, карточку ребёнка или балл теста, — сотрудник обязан написать причину. Причина короче восьми символов не принимается, и обращение не открывается. Мы не подставляем «не указана» автоматически: журнал, заполненный машиной, выглядит проверенным, будучи непроверяемым.\n\nОтветы теста на послеродовую депрессию не видит никто, включая нас: они не сохраняются. Виден только балл, и только на карточке конкретного человека — по нему нельзя ни отсортировать список, ни собрать выборку.\n\nРассылки не строятся по вашим показателям, дневнику или баллу. Мы действительно используем для них язык, срок беременности и возраст ребёнка — чтобы письмо о прикорме не пришло беременной. Правило, обращающееся к давлению, система отклоняет.',
    AppLocale.kk: 'Қызметкер тек өз жұмысына қажеттісін ғана көреді, ал құқықтар лауазым бойынша емес, міндет бойынша беріледі.\n\nСатушы мен қойма денсаулық туралы бірде-бір жазбаны көрмейді. Бухгалтерия ақшаны көреді, адамдарды көрмейді. Дәрігер денсаулықты көреді, ақшаны көрмейді.\n\nНақты бір әйелдің картасын — ұйқысын, цикл күнделігін, дәрілерін, бала картасын немесе тест балын — ашу үшін қызметкер себебін жазуға міндетті. Сегіз таңбадан қысқа себеп қабылданбайды, карта ашылмайды. Біз «көрсетілмеген» дегенді автоматты түрде қоймаймыз: машина толтырған журнал тексерілген болып көрінеді, ал шын мәнінде оны тексеру мүмкін емес.\n\nБосанғаннан кейінгі депрессия тестінің жауаптарын ешкім көрмейді, біз де көрмейміз: олар сақталмайды. Тек балл ғана көрінеді, әрі ол тек нақты адамның картасында ғана — ол бойынша тізімді сұрыптауға да, іріктеме жинауға да болмайды.\n\nТаратылымдар көрсеткіштеріңіз, күнделігіңіз немесе балыңыз бойынша құрылмайды. Біз олар үшін тілді, жүктілік мерзімін және баланың жасын шынымен пайдаланамыз — қосымша тамақ туралы хат жүкті әйелге келіп қалмауы үшін. Қан қысымына жүгінетін ережені жүйе қабылдамайды.',
    AppLocale.en: 'A member of staff sees only what their work requires, and rights are granted by task rather than by job title.\n\nSales and warehouse staff see no health record at all. Finance sees money and does not see people. A clinician sees health and does not see money.\n\nTo open a particular woman’s record — her sleep, cycle diary, medications, her child’s card or her screening score — a member of staff must write a reason. A reason shorter than eight characters is refused and the record does not open. We do not auto-fill “not stated”: a log filled in by a machine looks reviewed while being unreviewable.\n\nNobody sees the answers to the postnatal depression questionnaire, ourselves included: they are not stored. Only the score is visible, and only on an individual’s record — it cannot be sorted on or used to build a list.\n\nMessage audiences are not built from your readings, your diary or your score. We do use language, stage of pregnancy and a child’s age — so that a message about weaning does not reach someone who is pregnant. A rule that refers to blood pressure is rejected by the system.',
  },
  'legal_priv_age_h': {AppLocale.ru: 'Возраст', AppLocale.kk: 'Жас шамасы', AppLocale.en: 'Age'},
  'legal_priv_age_b': {
    AppLocale.ru: 'Приложением можно пользоваться с 16 лет.\n\nМы выбрали 16, а не 18, сознательно. Беременность до восемнадцати существует, и именно этим женщинам сопровождение нужно больше всего; порог в 18 лет не остановил бы их, а лишь заставил бы указать неверный возраст и остаться без поддержки.\n\nЕсли вам от 16 до 18, обработка ваших данных должна происходить с ведома родителя или законного представителя.\n\nДанные ребёнка вносите вы как взрослый. Сам ребёнок не является пользователем приложения: у брелока нет ни экрана, ни аккаунта.',
    AppLocale.kk: 'Қолданбаны 16 жастан бастап пайдалануға болады.\n\nБіз 18-ді емес, 16-ны әдейі таңдадық. Он сегізге толмай жүкті болу деген бар, әрі қолдау дәл сол әйелдерге бәрінен де қажет; 18 жас шегі оларды тоқтатпас еді, тек жасын жалған көрсетіп, қолдаусыз қалуға мәжбүрлер еді.\n\nЕгер жасыңыз 16 бен 18 аралығында болса, деректеріңізді өңдеу ата-ананың немесе заңды өкілдің хабардар болуымен жүруге тиіс.\n\nБала деректерін ересек адам ретінде сіз енгізесіз. Баланың өзі қолданбаның пайдаланушысы емес: брелокта экран да, аккаунт та жоқ.',
    AppLocale.en: 'The app may be used from the age of 16.\n\nWe chose 16 rather than 18 deliberately. Pregnancy under eighteen exists, and those are the women who most need support; an 18 threshold would not have stopped them, only made them state a false age and go without help.\n\nIf you are between 16 and 18, the processing of your data should happen with the knowledge of a parent or legal guardian.\n\nA child’s data is entered by you, as an adult. The child is not themselves a user of the app: the tag has neither a screen nor an account.',
  },
  'legal_priv_security_h': {AppLocale.ru: 'Как мы защищаем данные', AppLocale.kk: 'Деректерді қалай қорғаймыз', AppLocale.en: 'How we protect the data'},
  'legal_priv_security_b': {
    AppLocale.ru: 'Связь с сервером идёт по защищённому протоколу. Пароли и коды подтверждения хранятся не в открытом виде. Каждое обращение сотрудника к карточке человека записывается в журнал.\n\nСовершенной защиты не существует, и мы этого не обещаем. Если данные всё же утекут, мы сообщим об этом пострадавшим и уполномоченному органу, а не будем молчать.',
    AppLocale.kk: 'Сервермен байланыс қорғалған хаттама арқылы жүреді. Құпиясөздер мен растау кодтары ашық түрде сақталмайды. Қызметкердің адам картасына жасаған әрбір қаралымы журналға жазылады.\n\nМінсіз қорғаныс деген жоқ, біз оны уәде де етпейміз. Деректер бәрібір сыртқа шығып кетсе, үндемей қалмай, зардап шеккендерге және уәкілетті органға хабарлаймыз.',
    AppLocale.en: 'Communication with the server uses a secure protocol. Passwords and confirmation codes are not stored in the clear. Every staff access to an individual’s record is written to a log.\n\nPerfect security does not exist and we do not promise it. If data does leak, we will tell those affected and the authorised body rather than keep quiet.',
  },
  'legal_priv_changes_h': {AppLocale.ru: 'Изменения политики', AppLocale.kk: 'Саясаттың өзгеруі', AppLocale.en: 'Changes to this policy'},
  'legal_priv_changes_b': {
    AppLocale.ru: 'Мы обновляем этот текст, когда меняется приложение. Дата редакции стоит наверху.\n\nЕсли изменение существенно — например, мы начнём отправлять на сервер маршрут ребёнка или подключим уведомления, — мы скажем об этом в приложении до того, как оно вступит в силу, а не после.',
    AppLocale.kk: 'Қолданба өзгергенде осы мәтінді жаңартып отырамыз. Редакция күні жоғарыда тұр.\n\nЕгер өзгеріс елеулі болса — мысалы, баланың жүрген жолын серверге жібере бастасақ немесе хабарламаларды қоссақ — біз бұл туралы қолданбада ол күшіне енгенге дейін айтамыз, кейін емес.',
    AppLocale.en: 'We update this text when the app changes. The version date is at the top.\n\nIf a change is material — for instance, if we begin sending a child’s route to the server, or connect notifications — we will say so in the app before it takes effect, not after.',
  },
  ...
  'legal_terms_who_b': {
    AppLocale.ru: 'Пользоваться приложением можно с 16 лет. Если вам от 16 до 18, это должно происходить с ведома родителя или законного представителя.\n\nВнося данные ребёнка, вы подтверждаете, что являетесь его родителем или законным представителем и вправе это делать. Открывая доступ второму взрослому, вы отвечаете за свой выбор: доступ можно отозвать, но увиденное вернуть нельзя.',
    AppLocale.kk: 'Қолданбаны 16 жастан бастап пайдалануға болады. Жасыңыз 16 бен 18 аралығында болса, бұл ата-ананың немесе заңды өкілдің хабардар болуымен жүруге тиіс.\n\nБала деректерін енгізе отырып, оның ата-анасы немесе заңды өкілі екеніңізді және бұған құқығыңыз бар екенін растайсыз. Екінші ересек адамға рұқсат ашқанда, өз таңдауыңыз үшін жауап бересіз: рұқсатты кері қайтаруға болады, бірақ көрілгенді қайтару мүмкін емес.',
    AppLocale.en: 'The app may be used from the age of 16. If you are between 16 and 18, this should happen with the knowledge of a parent or legal guardian.\n\nBy entering a child’s data you confirm that you are their parent or legal guardian and are entitled to do so. By granting access to a second adult you take responsibility for that choice: access can be revoked, but what has been seen cannot be unseen.',
  },
  'legal_terms_account_h': {AppLocale.ru: 'Аккаунт и номер телефона', AppLocale.kk: 'Аккаунт және телефон нөмірі', AppLocale.en: 'Account and phone number'},
  'legal_terms_account_b': {
    AppLocale.ru: 'Аккаунт привязан к номеру телефона, вход — по коду из сообщения. Храните доступ к своему номеру: кто получит код, тот получит и вашу медицинскую карту, и данные ребёнка.\n\nЕсли вы смените номер, сообщите нам до того, как старый номер перейдёт другому человеку.\n\nОплаченный доступ к курсу и пройденные уроки привязаны к номеру, а не к аккаунту.',
    AppLocale.kk: 'Аккаунт телефон нөміріне байланған, кіру — хабарламадан келген код арқылы. Өз нөміріңізге қолжетімділікті сақтаңыз: кодты кім алса, сіздің медициналық картаңызды да, бала деректеріңізді де сол алады.\n\nНөміріңізді ауыстырсаңыз, ескі нөмір басқа адамға өтіп кетпей тұрып бізге хабарлаңыз.\n\nКурсқа төленген қолжетімділік пен өтілген сабақтар аккаунтқа емес, нөмірге байланған.',
    AppLocale.en: 'Your account is tied to a phone number, and you sign in with a code from a message. Keep control of your number: whoever receives the code receives your medical record and your child’s data too.\n\nIf you change your number, tell us before the old one passes to somebody else.\n\nPaid course access and completed lessons are tied to the number rather than to the account.',
  },
  'legal_terms_device_h': {AppLocale.ru: 'Браслет и брелок', AppLocale.kk: 'Білезік және брелок', AppLocale.en: 'The band and the tag'},
  'legal_terms_device_b': {
    AppLocale.ru: 'Устройства подключаются по Bluetooth и работают рядом с телефоном. Пока телефон вне зоны действия, устройство копит данные и передаёт их при следующем соединении — поэтому показания могут появиться с задержкой.\n\nГарантия на устройство и его замена регулируются условиями покупки. Приложение бесплатно для владельца устройства, а без устройства часть функций недоступна.\n\nМы не отвечаем за работу устройства, купленного не у нас или изменённого сторонней прошивкой.',
    AppLocale.kk: 'Құрылғылар Bluetooth арқылы жалғанады және телефонның қасында жұмыс істейді. Телефон қолжетім аймағынан тыс тұрғанда құрылғы деректерді жинап отырады да, келесі қосылғанда жібереді — сондықтан көрсеткіштер кешігіп пайда болуы мүмкін.\n\nҚұрылғыға берілетін кепілдік және оны ауыстыру сатып алу шарттарымен реттеледі. Қолданба құрылғы иесі үшін тегін, ал құрылғысыз кейбір функциялар қолжетімсіз.\n\nБізден алынбаған немесе бөгде микробағдарламамен өзгертілген құрылғының жұмысы үшін жауап бермейміз.',
    AppLocale.en: 'The devices connect over Bluetooth and work near the phone. While the phone is out of range the device accumulates data and sends it at the next connection — so readings may appear with a delay.\n\nWarranty and replacement are governed by the purchase terms. The app is free for the owner of a device, and without a device some features are unavailable.\n\nWe are not responsible for a device bought elsewhere or altered with third-party firmware.',
  },
  'legal_terms_paid_h': {AppLocale.ru: 'Платные материалы', AppLocale.kk: 'Ақылы материалдар', AppLocale.en: 'Paid material'},
  'legal_terms_paid_b': {
    AppLocale.ru: 'Часть материалов — например, курс для мам — открывается после покупки. Доступ привязан к номеру телефона и не ограничен по времени, пока сервис работает.\n\nЕсли мы удалим или заменим урок, вы не теряете доступ к курсу целиком.',
    AppLocale.kk: 'Кейбір материалдар — мысалы, аналарға арналған курс — сатып алғаннан кейін ашылады. Қолжетімділік телефон нөміріне байланған және сервис жұмыс істеп тұрғанда уақытпен шектелмейді.\n\nЕгер біз сабақты жойсақ немесе ауыстырсақ, курсқа қолжетімділікті түгел жоғалтпайсыз.',
    AppLocale.en: 'Some material — the course for mothers, for instance — opens after purchase. Access is tied to your phone number and is not time-limited for as long as the service runs.\n\nIf we remove or replace a lesson, you do not lose access to the course as a whole.',
  },
  'legal_terms_orders_h': {AppLocale.ru: 'Заказы и доставка', AppLocale.kk: 'Тапсырыстар және жеткізу', AppLocale.en: 'Orders and delivery'},
  'legal_terms_orders_b': {
    AppLocale.ru: 'Заказ оформляется через WhatsApp или по ссылке на Kaspi. Оплата проходит на стороне банка или платёжного сервиса: мы не получаем и не храним данные вашей карты.\n\nЦены, сроки и условия доставки сообщаются при оформлении. Возврат и обмен — по законодательству Республики Казахстан о защите прав потребителей.\n\nЧтобы доставить заказ, нам нужны ваше имя, телефон и адрес. Эти сведения остаются у нас и после удаления аккаунта — по причине, описанной в политике конфиденциальности.',
    AppLocale.kk: 'Тапсырыс WhatsApp арқылы немесе Kaspi сілтемесімен ресімделеді. Төлем банктің немесе төлем сервисінің жағында өтеді: біз карта деректеріңізді алмаймыз да, сақтамаймыз да.\n\nБағалар, мерзімдер және жеткізу шарттары ресімдеу кезінде хабарланады. Қайтару мен айырбастау — Қазақстан Республикасының тұтынушылардың құқықтарын қорғау туралы заңнамасына сәйкес.\n\nТапсырысты жеткізу үшін бізге атыңыз, телефоныңыз және мекенжайыңыз керек. Бұл мәліметтер аккаунт жойылғаннан кейін де бізде қалады — құпиялылық саясатында сипатталған себеппен.',
    AppLocale.en: 'An order is placed over WhatsApp or through a Kaspi link. Payment happens on the bank’s or the payment service’s side: we neither receive nor store your card details.\n\nPrices, timescales and delivery terms are given at the point of order. Returns and exchanges follow the consumer protection legislation of the Republic of Kazakhstan.\n\nTo deliver an order we need your name, phone and address. That information stays with us after the account is deleted, for the reason described in the privacy policy.',
  },
  'legal_terms_content_h': {AppLocale.ru: 'Ваши записи', AppLocale.kk: 'Сіздің жазбаларыңыз', AppLocale.en: 'Your entries'},
  'legal_terms_content_b': {
    AppLocale.ru: 'Всё, что вы вносите, остаётся вашим. Мы не публикуем ваши записи, не показываем их другим пользователям и не используем для рекламы.\n\nМы храним и обрабатываем их только для того, чтобы приложение работало: показывало вам ваши данные, возвращало их на новом телефоне и вовремя предупреждало о тревожных показателях.',
    AppLocale.kk: 'Енгізгеніңіздің бәрі өзіңіздікі болып қалады. Жазбаларыңызды жарияламаймыз, басқа пайдаланушыларға көрсетпейміз және жарнама үшін пайдаланбаймыз.\n\nОларды тек қолданба жұмыс істеуі үшін ғана сақтаймыз және өңдейміз: деректеріңізді сізге көрсету, жаңа телефонда қайтару және қауіпті көрсеткіштер туралы уақытында ескерту үшін.',
    AppLocale.en: 'Everything you enter remains yours. We do not publish your entries, do not show them to other users and do not use them for advertising.\n\nWe store and process them only so that the app works: to show you your own data, to return it to you on a new phone, and to warn you in time about concerning readings.',
  },
  'legal_terms_prohibited_h': {AppLocale.ru: 'Чего делать нельзя', AppLocale.kk: 'Не істеуге болмайды', AppLocale.en: 'What you may not do'},
  'legal_terms_prohibited_b': {
    AppLocale.ru: '• Вносить данные человека, представителем которого вы не являетесь, и следить за взрослым без его ведома.\n• Пытаться получить доступ к чужому аккаунту или к внутренней части сервиса.\n• Вскрывать, копировать или изменять приложение и его протокол обмена с устройством.\n• Использовать сервис для действий, запрещённых законом.\n\nМы можем закрыть доступ, если увидим такое, и обязаны сообщить в органы, если речь идёт о безопасности ребёнка.',
    AppLocale.kk: '• Өзіңіз өкілі болып табылмайтын адамның деректерін енгізу және ересек адамды оның хабарынсыз қадағалау.\n• Бөтен аккаунтқа немесе сервистің ішкі бөлігіне қол жеткізуге әрекеттену.\n• Қолданбаны және оның құрылғымен алмасу хаттамасын ашу, көшіру немесе өзгерту.\n• Сервисті заңмен тыйым салынған әрекеттер үшін пайдалану.\n\nМұндайды байқасақ, қолжетімділікті жабуымыз мүмкін, ал әңгіме баланың қауіпсіздігі туралы болса, органдарға хабарлауға міндеттіміз.',
    AppLocale.en: '• Enter the data of a person you do not represent, or track an adult without their knowledge.\n• Attempt to access another person’s account or the internal parts of the service.\n• Reverse-engineer, copy or modify the app and its device protocol.\n• Use the service for anything the law forbids.\n\nWe may close access if we see this, and we are obliged to report to the authorities where a child’s safety is concerned.',
  },
  'legal_terms_availability_h': {AppLocale.ru: 'Доступность сервиса', AppLocale.kk: 'Сервистің қолжетімділігі', AppLocale.en: 'Availability of the service'},
  'legal_terms_availability_b': {
    AppLocale.ru: 'Мы стараемся, чтобы сервис работал постоянно, но не обещаем непрерывной работы. Возможны перерывы на обслуживание, сбои связи и ошибки.\n\nМы можем изменять функции приложения и прекращать отдельные из них. О существенных изменениях предупредим в приложении.',
    AppLocale.kk: 'Сервис үнемі жұмыс істеп тұруы үшін тырысамыз, бірақ үзіліссіз жұмысты уәде етпейміз. Техникалық қызмет көрсетуге байланысты үзілістер, байланыс ақаулары және қателер болуы мүмкін.\n\nҚолданба функцияларын өзгертуіміз және жекелегендерін тоқтатуымыз мүмкін. Елеулі өзгерістер туралы қолданбада ескертеміз.',
    AppLocale.en: 'We aim for the service to run continuously but do not promise uninterrupted operation. Maintenance breaks, connection failures and errors are possible.\n\nWe may change the app’s features and discontinue individual ones. We will give notice of material changes in the app.',
  },
  'legal_terms_termination_h': {AppLocale.ru: 'Прекращение', AppLocale.kk: 'Тоқтату', AppLocale.en: 'Ending'},
  'legal_terms_termination_b': {
    AppLocale.ru: 'Вы можете прекратить пользоваться приложением в любой момент, удалив аккаунт в настройках.\n\nМы можем закрыть доступ при существенном нарушении этих условий. Если это произойдёт, мы объясним причину и дадим забрать копию ваших данных.',
    AppLocale.kk: 'Қолданбаны пайдалануды кез келген сәтте параметрлерден аккаунтты жою арқылы тоқтата аласыз.\n\nОсы шарттар елеулі түрде бұзылған жағдайда қолжетімділікті жабуымыз мүмкін. Егер бұлай болса, себебін түсіндіреміз және деректеріңіздің көшірмесін алуға мүмкіндік береміз.',
    AppLocale.en: 'You may stop using the app at any time by deleting your account in Settings.\n\nWe may close access in the event of a material breach of these terms. If that happens we will explain why and let you take a copy of your data.',
  },
  'legal_terms_changes_h': {AppLocale.ru: 'Изменения условий', AppLocale.kk: 'Шарттардың өзгеруі', AppLocale.en: 'Changes to these terms'},
  'legal_terms_changes_b': {
    AppLocale.ru: 'Мы можем менять эти условия. Дата редакции стоит наверху, а о существенных изменениях мы скажем в приложении.\n\nЕсли вы продолжаете пользоваться приложением после изменения, считается, что вы его приняли.',
    AppLocale.kk: 'Осы шарттарды өзгертуіміз мүмкін. Редакция күні жоғарыда тұр, ал елеулі өзгерістер туралы қолданбада айтамыз.\n\nӨзгерістен кейін қолданбаны пайдалануды жалғастырсаңыз, оны қабылдадыңыз деп саналады.',
    AppLocale.en: 'We may change these terms. The version date is at the top, and we will announce material changes in the app.\n\nIf you continue using the app after a change, you are taken to have accepted it.',
  },
  ...
  // And the same absence PARTIALLY: some cards of the grid were assessed and
  // some were not. The everyday state of a band user, because a wrist
  // temperature and a wrist blood pressure may not be graded — so the ring
  // spent most days drawing a complete circle over two cards of four and saying
  // nothing about the other two.
  //
  // It names a count and not a feeling. «{n} из {total}» is arithmetic the
  // reader can check by counting the cards below it, which is why the ring
  // counts the four CARDS of the grid and not the five series behind them.
  //
  // Neither an alarm nor a reassurance, and the wording was chosen against both
  // failures: it says nothing about her body at all. «Учтены не все» is about
  // what the app did, and the negation carries no verb she has to act on —
  // «недостаточно данных» would read as a fault of hers, and «всё в порядке по
  // 2 из 4» would be the reassurance this whole review exists to remove.
  //
  // The plural is dodged rather than solved: «показатели» is fixed text and
