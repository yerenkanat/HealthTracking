...
SetupProgress p({
  ...
  bool backup = false,
    ...
    computeSetupProgress(
      hasName: name, hasHealthData: health, hasChild: child, hasZone: zone, hasBackup: backup,
    );
...
void main() {
  ...
  final all = p(name: true, health: true, child: true, zone: true, backup: true);
  ...
  _chk('partial done count', some.done.length == 2 && some.remaining.length == 3);
  _chk('partial fraction', (some.fraction - 0.4).abs() < 1e-9);
  _chk('next skips completed steps', some.next == SetupStep.child);
  _chk('remaining keeps declaration order',
      some.remaining[0] == SetupStep.child && some.remaining[1] == SetupStep.zone && some.remaining[2] == SetupStep.backup);
